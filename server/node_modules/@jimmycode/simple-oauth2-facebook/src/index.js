  ...
  */
  create: (config) => {
    ...
    requiredArgs.forEach((arg) => {
      if (!args[arg.name]) {
        throw new Error(`'SimpleOAuth2Facebook requires a [${arg.name}]'`);
      }
      // eslint-disable-next-line valid-typeof
      else if (typeof args[arg.name] !== arg.type) {
        // eslint-disable-next-line max-len
        throw new Error(`SimpleOAuth2Facebook expects [${arg.name}] to be a [${arg.type}] but it was a [${typeof args[arg.name]}]'`);
      }
    });
    ...
    const oauth2 = simpleOAuth2.create({
      ...
      auth: {
        authorizeHost: args.authorizeHost || 'https://facebook.com',
        authorizePath: args.authorizePath || '/dialog/oauth',
        tokenHost: args.tokenHost || 'https://graph.facebook.com',
    ...
    const accessToken = (req, res, next) => {
      ...
      if (!code) {
        const error = new Error('SimpleOAuth2Facebook expects [code] param in the request');
