    ...
    return (
      <React.Fragment>
        ...
        <Modal isActive={isActive}>
          ...
          <Modal.Card>
            <Modal.Card.Head>
              <Modal.Card.Title>WeatherUV widget settings</Modal.Card.Title>
...
export default class WeatherUVPage extends React.Component {
  constructor(props){
      super(props)
      this.state = {
        activeDrags: 0,
  ...
  async handleSubmit() {
    ...
    await axios({
      method: 'post',
      url: 'http://localhost:7070/api/weatherUV',
      data: JSON.stringify({
      ...
      if (response.data === 'error')
        this.setState({uv: 'Undefined'})
      else {
        this.setState({lunchTimer: true})
        this.setState({uv: response.data.value}, () => {this.changeUv()})
      }
    });
  }
  changeUv = () => {
    if (this.state.uv > 8.0)
        this.setState({message: 'très rapide'})
    else if (this.state.uv > 6.0)
        this.setState({message: 'rapide'})
    else if (this.state.uv > 4.0)
        this.setState({message: 'modéré'})
    else if (this.state.uv > 2.0)
        this.setState({message: 'presque pas'})
    else
        this.setState({message: 'aucun'})
  }
  onStart = () => {
      ...
      if (this.state.lunchTimer === true) {
        await axios({
          ...
          if (response.data === 'error')
            ...
            this.setState({uv: response.data.value})
        });
      }
