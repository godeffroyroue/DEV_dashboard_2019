...
class ChannelDetail extends React.Component {
  ...
  handleButton = async () => {
    await axios({
      method: 'post',
      url: 'http://localhost:7070/api/youtube/channel',
      data:{
        termFromSearchBar: this.props.channel.snippet.channelId
      },
    }).then((response) => {
      this.setState({selectedChannel: response.data.items[0]})
      this.setState({nbSubscibe: this.state.selectedChannel.statistics.subscriberCount})
      this.setState({nbVideo: this.state.selectedChannel.statistics.videoCount})
      this.setState({nbView: this.state.selectedChannel.statistics.viewCount})
        ...
        <Column>
            <div>
              <div className='ui segment'>
                  <Tag isMedium isPrimary>About this Channel</Tag>
                  <Label><Tag isInfo>Name</Tag> {this.props.channel.snippet.title}</Label>
                  <Label><Tag isInfo>Subscribe</Tag> {this.state.nbSubscibe}</Label>
                  <Label><Tag isInfo>Videos</Tag> {this.state.nbVideo}</Label>
                  <Label><Tag isInfo>View</Tag> {this.state.nbView}</Label>
              </div>
            </div>
        </Column>
      </Columns>
